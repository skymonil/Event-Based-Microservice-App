...
describe("Order Service - Full E2E (Sagas & Edge Cases)", () => {
	...
	it("should forbid accessing another users order", async () => {
		...
		await hackerClient.post("/api/users", {
			email: hackerEmail,
			password: "pw",
			firstName: "H",
			lastName: "K",
		});
		const hackerLogin = await hackerClient.post("/api/login", {
			email: hackerEmail,
			password: "pw",
		});
	...
	it("should execute full Refund Saga for PAID orders", async () => {
		...
		// Move to PAID first
		await kafkaClient.produce("payment.completed", orderId, {
			orderId,
			status: "SUCCESS",
		});
		await waitFor(async () => {
		...
		await waitFor(
			async () => {
				return !!kafkaClient.findMessage(
					"order.events",
					(m) =>
			...
			10000,
			"Cancel Requested event missing",
		);

		// Simulate Refund Completion
		await kafkaClient.produce("payment.refunded", orderId, {
			orderId,
			status: "REFUNDED",
		});
		...
		await waitFor(
			...
			10000,
			"Final CANCELLED status missing",
		);
	});
