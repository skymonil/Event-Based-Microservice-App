	...
	return tracer.startActiveSpan("order.cancel", async (span) => {
		...
		try {
			...
			if (order.user_id !== userId) {
				logger.error(
					logContext,
					"Security Alert: User attempted to cancel an order they do not own",
				);
			...
			if (new Date() > order.cancellable_until) {
				logger.warn(
					{ ...logContext, cancellableUntil: order.cancellable_until },
			...
			if (order.status === "CREATED" || order.status === "PAYMENT_FAILED") {
				...
				logger.info(
					logContext,
					`Order state ${order.status} -> CANCELLED (No refund needed)`,
				);
				...
				logger.info(
					logContext,
					"Order state PAID -> CANCELLING (Refund Saga triggered)",
				);
			} else {
				logger.warn(
					{ ...logContext, currentStatus: order.status },
					"Cancellation rejected: Invalid state",
				);
				throw new AppError({
					status: 409,
...
const handlePaymentCompleted = async (orderId, paymentId, eventId) => {
	return tracer.startActiveSpan(
		"order.handlePaymentCompleted",
		async (span) => {
			span.setAttribute("order.id", orderId);
			...
			try {
				...
				if (!isNewEvent) {
					...
					await client.query("ROLLBACK"); // Rollback, but don't throw an error!
					span.setStatus({ code: SpanStatusCode.OK });
					return; // Acknowledge the message to Kafka
				}
...
const handlePaymentFailed = async (orderId, paymentId, reason) => {
	...
	return tracer.startActiveSpan("order.handlePaymentFailed", async (span) => {
		...
		span.setAttribute("payment.id", paymentId);
		try {
			await orderQueries.updateOrderStatus(orderId, "PAYMENT_FAILED");
...
const handlePaymentRefunded = async ({ orderId, traceHeaders }) => {
	...
	try {
		...
		await orderQueries.markCancelled(
			orderId,
